...
impl Network {
            ...
            credentials: {
                match credentials_input {
                    ...
                    Some(credentials_input) => {
                        serde_json::from_str::<Credentials>(credentials_input).ok()
                    }
                }
            },
        }
    ...
    pub fn save(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        klave::ledger::get_table(NETWORK_MANAGER_TABLE)
            .set(self.name.as_str(), serialized_network.as_bytes())?;
        Ok(())
    }
    ...
    {
        let http_request: Request<String> = if self.credentials.is_some() {
            let token = self.generate_token()?;
            http::request_format_with_auth(self.get_rpc_url(), body, &token)?
        } else {
            http::request_format(self.get_rpc_url(), body)?
        };
        ...
        let result = match klave::https::request(&http_request) {
            ...
            Err(e) => {
                klave::notifier::send_string(&format!(
                    "ERROR: request - failed to send request: {}, {:?}, {}, {}",
                    &http_request.uri(),
        ...
        let tx_response = match http::parse_json_rpc_response::<T>(result.body()) {
            ...
            Err(e) => {
                klave::notifier::send_string(&format!(
                    "ERROR: request - failed to parse response: {}, {}, {}, {:?}, {}",
                    result.body(),
                    e,
                    &http_request.uri(),
                    &http_request.headers(),
