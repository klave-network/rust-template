...
impl PersistCommand {
    ...
    >(
        ...
        let _ = match self.bootstrap_info {
            Some(bootstrap_info) => {
                lc.store_boostrap(match serde_json::from_str(&bootstrap_info) {
        ...
        let _ = match self.state_info {
            Some(state_info) => {
                lc.store_light_client_state(match serde_json::from_str(&state_info) {
                    Ok(state) => state,
                    Err(e) => {
