...
pub fn generate_secp256k1_secret_key() -> Result<String, Box<dyn std::error::Error>> {
    let array_rng: [u8; 32] = crypto::random::get_random_bytes(32)?
        .as_slice()
        .try_into()?;
    let seckey = format!(
        "{}",
        SecretKey::from_byte_array(&array_rng)?.display_secret()
    );
    Ok(seckey)
}

pub fn generate_public_key_from_secret_key(
    seckey: String,
...
pub fn generate_rnd_bytes() -> Result<[u8; 32], Box<dyn std::error::Error>> {
    let rnd_bytes: [u8; 32] = crypto::random::get_random_bytes(32)?
        .as_slice()
        .try_into()?;
    Ok(rnd_bytes)
}
