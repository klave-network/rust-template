...
use crate::utils::{
    generate_public_key_from_secret_key, generate_rng_id, generate_secp256k1_secret_key,
};
...
pub fn register_user(cmd: String) {
    let Ok(res) = serde_json::from_str::<InputRegistration>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as InputRegistration"
        ));
        return;
    };
    ...
    if let Ok(user) = klave::ledger::get_table("users_tab").get_string(&sender_id) {
        klave::notifier::send_string(&format!("ERROR: user already registered: '{user}'"));
        return;
    }
    if let Err(e) = klave::ledger::get_table("users_tab").set_string(&sender_id, &res.user_name) {
        klave::notifier::send_string(&format!("ERROR: failed to register user: '{e}'"));
        return;
    }
    if let Err(e) = klave::ledger::get_table("users_id_tab").set_string(&res.user_name, &sender_id)
    {
    ...
    match klave::ledger::get_table("users_id_tab").set_json::<Vec<String>>("ALL", &username_list) {
        Err(e) => {
            klave::notifier::send_string(&format!(
                "ERROR: failed to register global users list '{e}'"
            ));
        }
        Ok(()) => {
...
pub fn load_current_user(_cmd: String) {
    ...
    klave::notifier::send_string(&sender_id);
}
...
pub fn create_key(cmd: String) {
    ...
    let Ok(res) = serde_json::from_str::<InputKeyCreation>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as InputKeyCreation"
        ));
        return;
    };
    //get sender id
    ...
    let mut user_key_ids_map: Map<String, Value> =
        match klave::ledger::get_table("user_key_ids_tab").get_json(&sender_id) {
            Ok(json) => json,
            Err(_) => Map::new(),
        };
    if user_key_ids_map.contains_key(&res.key_name) {
        klave::notifier::send_string(&format!(
            "ERROR: key with name '{}' already exists",
            res.key_name
        ));
        return;
    }
    ...
    if let Err(e) =
        klave::ledger::get_table("user_key_ids_tab").set_json(&sender_id, &user_key_ids_map)
    {
        klave::notifier::send_string(&format!("ERROR: failed to register user: '{e}'"));
        return;
    }
    ...
    if let Err(e) = klave::ledger::get_table("users_keys_tab").set_json(&key_id, &key_pair) {
...
pub fn load_keys(_cmd: String) {
    ...
    let Ok(list_user_keys) =
        klave::ledger::get_table("user_key_ids_tab").get_json::<Map<String, Value>>(&sender_id)
    else {
...
pub fn load_key_pair(cmd: String) {
    ...
    let Ok(res) = serde_json::from_str::<KeyId>(&cmd) else {
        klave::notifier::send_string(&format!("ERROR: failed to parse '{cmd}' as KeyId"));
        return;
    };
    //get key pair
    let Ok(key_pair_string) =
        klave::ledger::get_table("users_keys_tab").get_json::<KeyPairString>(&res.key_id)
    else {
...
pub fn create_musig_session_definition(cmd: String) {
    ...
    let Ok(res) = serde_json::from_str::<InputMusigSessionInvitation>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as InputMusigSessionInvitation"
        ));
        return;
    };
    //get sender id
    ...
    //generate session id
    let Ok(musig_session_id) = crypto::random::get_random_bytes(32).map(hex::encode) else {
    ...
    for element in res.participants {
        let Ok(senderid) = klave::ledger::get_table("users_id_tab").get_string(&element) else {
            klave::notifier::send_string(&format!(
                "ERROR: failed to find senderid for '{element}'"
            ));
            return;
        };
    ...
    let Ok(_) =
        klave::ledger::get_table("musig_sessions_def_tab").set_json(&musig_session_id, &session)
    else {
    ...
    for elem in sender_id_list {
        let mut list_musig_sessions: Vec<String> =
            klave::ledger::get_table("musig_user_sessions_tab")
                .get_json(&elem)
                .unwrap_or_default();
        list_musig_sessions.push(musig_session_id.clone());
        if let Err(e) = klave::ledger::get_table("musig_user_sessions_tab")
            .set_json(&elem, &list_musig_sessions)
        {
            klave::notifier::send_string(&format!(
                "ERROR: failed to save user musig sessions: '{e}'"
            ));
            return;
        }
    }
    //return musig session id
    let output_musig_session_id = MusigSessionId { musig_session_id };
...
pub fn update_musig_user_public_key(cmd: String) {
    let Ok(res) = serde_json::from_str::<InputMusigSessionUserPubKey>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as InputMusigSessionUserPubKey"
        ));
        return;
    };
    let Ok(sender_id) = klave::context::get("sender") else {
    ...
    let Ok(mut session) = klave::ledger::get_table("musig_sessions_def_tab")
        .get_json::<MusigSessionDefinition>(&res.musig_session_id)
    else {
    ...
    //retrieve list of user keys
    let user_key_ids_map: Map<String, Value> =
        match klave::ledger::get_table("user_key_ids_tab").get_json(&sender_id) {
            Ok(json) => json,
            Err(_) => Map::new(),
        };
    ...
    //retrieve key id
    let key_id = match user_key_ids_map.get(&res.key_name) {
    ...
    let key_pair_string: KeyPairString =
        match klave::ledger::get_table("users_keys_tab").get_json::<KeyPairString>(&key_id) {
            Ok(key) => key,
            Err(e) => {
                klave::notifier::send_string(&format!(
                    "ERROR: failed to load key pair with error '{e}'"
                ));
                return;
            }
        };
    ...
    session
        .public_keys
        .insert(sender_id.clone(), Value::String(key_pair_string.pub_key));
    ...
    let Ok(_) = klave::ledger::get_table("musig_sessions_def_tab")
        .set_json(&res.musig_session_id, &session)
    else {
        klave::notifier::send_string("ERROR: failed to save session");
        return;
    };
    ...
    let mut map_user_session_key = klave::ledger::get_table("musig_sessions_user_key_name")
        .get_json::<Map<String, Value>>(&sender_id)
        .unwrap_or_default();
    ...
    let Ok(_) = klave::ledger::get_table("musig_sessions_user_key_name")
        .set_json(&sender_id, &map_user_session_key)
    else {
...
pub fn update_musig_aggregation_session(cmd: String) {
    ...
    let Ok(res) = serde_json::from_str::<MusigSessionUpdateWithAggId>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as MusigSessionUpdateWithAggId"
        ));
        return;
    };
    ...
    match klave::ledger::get_table("musig_sessions_def_tab")
        .set_json(&res.musig_session_id, &session)
    {
        Ok(_) => {
            let _ = klave::notifier::send_json::<MusigSessionDefinition>(&session);
        ...
        Err(e) => {
            klave::notifier::send_string(&format!(
                "ERROR: failed to save musig session with error '{e}'"
            ));
        }
    }
}
...
pub fn load_musig_session_ids(_cmd: String) {
    ...
    let Ok(list_musig_sessions) =
        klave::ledger::get_table("musig_user_sessions_tab").get_json::<Vec<String>>(&sender_id)
    else {
...
pub fn load_musig_session(cmd: String) {
    let Ok(res) = serde_json::from_str::<MusigSessionId>(&cmd) else {
        klave::notifier::send_string(&format!("ERROR: failed to parse '{cmd}' as MusigSessionId"));
        return;
    };
    let Ok(sender_id) = klave::context::get("sender") else {
        klave::notifier::send_string("ERROR: failed to get the sender id");
    ...
    let Ok(session) = klave::ledger::get_table("musig_sessions_def_tab")
        .get_json::<MusigSessionDefinition>(&res.musig_session_id)
    else {
        klave::notifier::send_string("ERROR: failed to load session");
        return;
