...
pub fn create_public_nonce(cmd: String) {
    ...
    let Ok(res) = serde_json::from_str::<InputNonceCreation>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as InputNonceCreation"
        ));
        return;
    };
    //get sender id
    ...
    let mut map_musig_seed_nonce = klave::ledger::get_table("musig_user_seed_nonce_tab")
        .get_json::<Map<String, Value>>(&sender_id)
        .unwrap_or_default();
    map_musig_seed_nonce.insert(
        res.musig_session_id.clone(),
        Value::String(hex::encode(nonce_seed)),
    );
    let Ok(()) = klave::ledger::get_table("musig_user_seed_nonce_tab")
        .set_json::<Map<String, Value>>(&sender_id, &map_musig_seed_nonce)
    else {
    ...
    let signer_index = match session.signer_indexes {
        Some(value) => *value.get(&sender_id).unwrap(),
        None => {
            klave::notifier::send_string("ERROR: failed to load signer indexes");
            return;
        }
    ...
    let map_user_session_key = klave::ledger::get_table("musig_sessions_user_key_name")
        .get_json::<Map<String, Value>>(&sender_id)
        .unwrap_or_default();

    let key_name: String = match map_user_session_key.get(&res.musig_session_id) {
    ...
    //get the secret key
    let key_id = match user_key_ids_map.get(&key_name) {
        Some(value) => value.as_str().unwrap().to_string(),
        None => {
    ...
    let Ok(key_pair) =
        klave::ledger::get_table("users_keys_tab").get_json::<KeyPairString>(&key_id)
    else {
...
pub fn create_partial_signature(cmd: String) {
    ...
    //get nonce seed
    let map_musig_seed_nonce = klave::ledger::get_table("musig_user_seed_nonce_tab")
        .get_json::<Map<String, Value>>(&sender_id)
        .unwrap_or_default();
    ...
    let nonce_seed_str: String = match map_musig_seed_nonce.get(&res.musig_session_id) {
        Some(value) => value.as_str().unwrap().to_string(),
        None => {
    ...
    let signer_index: i8 = match session.signer_indexes {
        Some(value) => *value.get(&sender_id).unwrap(),
        None => {
            klave::notifier::send_string("ERROR: failed to load signer indexes");
            return;
    ...
    let key_id: String = match user_key_ids_map.get(&key_name) {
        Some(value) => value.as_str().unwrap().to_string(),
        None => {
            klave::notifier::send_string("ERROR: failed to get key id");
            return;
...
pub fn verify_final_signature(cmd: String) {
    ...
    let Ok(res) = serde_json::from_str::<FinalSignatureInput>(&cmd) else {
        klave::notifier::send_string(&format!(
            "ERROR: failed to parse '{cmd}' as FinalSignatureInput"
        ));
        return;
    };
    //deserialize key aggregated context
    ...
    let sig: [u8; 64] = hex::decode(res.final_signature)
        .unwrap()
        .try_into()
        .unwrap();
